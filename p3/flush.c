...
pid_t create_process(struct Process *p, int *index_stack, int *top, char *command) {
    pid_t pid;
    pid_t cpid, w;
    int status;
    pid = fork();
    ...
    if (!strcmp(argv[size - 1], "&")){
        add_process(p, pid, command, index_stack, top);
        return pid;
    } 
...
int main(void) {
    ...
    create_process(processes, index_stack, &top, str);
